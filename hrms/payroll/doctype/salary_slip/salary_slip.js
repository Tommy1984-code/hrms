...
frappe.ui.form.on("Salary Slip", {
	...
	set_dynamic_labels: function (frm) {
		if (frm.doc.employee && frm.doc.currency) {
			...
			// 	filters: {
			// 		employee: frm.doc.employee,
			// 		salary_structure: "Advance Salary Structure Asku"
			// 	},
			// 	fields: ['net_pay'],
			...
			// 		let advance_net_pay = salary_slips[0].net_pay;
			
			// 		// Check if "test Deduct" already exists
			// 		const existing_deductions = frm.doc.deductions || [];
			// 		const deduction_exists = existing_deductions.some(d => d.salary_component === "test Deduct");
			
			// 		// Only add if it doesn't already exist
			// 		if (!deduction_exists && frm.doc.salary_structure ==="Performance Salary Structure" ) {
			// 			const new_deduction = frm.add_child('deductions');
			// 			new_deduction.salary_component = "test Deduct"; // Specify the deduction component name
			// 			new_deduction.amount = advance_net_pay; // Set the amount to the net_pay value
			// 		}
			// 	}
			// });
			frappe.run_serially([
