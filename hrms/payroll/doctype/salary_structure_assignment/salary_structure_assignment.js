...
frappe.ui.form.on("Salary Structure Assignment", {
	...
	employee: function (frm) {
		if (frm.doc.employee) {
			...
			// my Code call server method to update payment type based on employee selection
            // frappe.call({
			// 	method:'hrms.payroll.doctype.salary_structure_assignment.salary_structure_assignment.set_payment_type_based_on_advance',
			// 	args:{
			...
			// 		if(r.message){
			// 			frm.set_value('payment_type',r.message);//updates the payment_type field
			// 			console.log(frm.doc.payment_type)
			// 		}
			// 		else{
			// 			console.error("No Payment type returned from server")
			// 		}
			// 	},
			// 	error:function(err){
	...
	from_date: function (frm) {
		if (frm.doc.from_date) {
			console.log("the payment type is :",frm.doc.payment_type)
			frm.trigger("toggle_opening_balances_section");
			frm.trigger('set_payment_type')
	...
	set_payment_type: function (frm) {
        ...
        // Make a call to the server to get the payment type based on both fields
        frappe.call({
            method: 'hrms.payroll.doctype.salary_structure_assignment.salary_structure_assignment.set_payment_type_based_on_advance',
            args: {
                employee: frm.doc.employee,
            ...
            callback: function (r) {
                if (r.message) {
                    frm.set_value('payment_type', r.message); // Set the payment type based on server response
                    console.log("Payment type set to:", r.message);
                } else {
                    console.error("No Payment type returned from server");
                }
                frm.refresh_field('payment_type'); // Refresh the field to see the change
            },
            error: function (err) {
                console.error("Error calling server method:", err);
            }
